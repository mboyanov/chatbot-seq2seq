...
def create_vocabulary(vocabulary_path, data_path, max_vocabulary_size,
  ...
  if not gfile.Exists(vocabulary_path):
    ...
    counter = 0
    for conversation in open(data_path, 'r').readlines()[1:]:
      utterances = udcparser.parse(conversation)
      if utterances is not None:
        counter += 1
        if counter % 100000 == 0:
          print("  processing line %d" % counter)
        tokenized_utterances = [tokenizer(utterance) if tokenizer else basic_tokenizer(utterance) for utterance in utterances]
        ...
        for conversation in open(data_path, 'r').readlines()[1:]:
          ...
          if utterances is not None:
            ...
            for i in range(len(utterances)-1):
              ut1 = utterances[i]
              ut2 = utterances[i+1]
              token_ids = sentence_to_token_ids(ut1, vocab, tokenizer, normalize_digits)
              questions_tokens_file.write(" ".join([str(tok) for tok in token_ids]) + "\n")
              token_ids = sentence_to_token_ids(ut2, vocab, tokenizer, normalize_digits)
              answers_tokens_file.write(" ".join([str(tok) for tok in token_ids]) + "\n")


def prepare_udc_data(data_dir, en_vocabulary_size, fr_vocabulary_size, tokenizer=None):
  ...
  create_vocabulary(vocab_path, '/home/martin/data/udc/train.csv', fr_vocabulary_size, tokenizer)
  ...
  data_to_token_ids('/home/martin/data/udc/train.csv', question_train_ids_path, answer_train_ids_path, vocab_path, tokenizer)
  data_to_token_ids('/home/martin/data/udc/test.csv', question_test_ids_path, answer_test_ids_path, vocab_path, tokenizer)
